...
/*   Updated: 2021/11/29 15:19:42 by hseong           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (fd < 0 || BUFFER_SIZE < 1 || gnl_load_buf(fd_buf, &buf, fd))
		return (NULL);
	line = (t_line){128, 0, (unsigned char *)malloc(sizeof(char) * 128)};
	if (!line.str)
		return (NULL);
	while (gnl_append(&line, buf) == 1)
	{
		buf->size = read(buf->fd * (buf->fd != INT_MAX), buf->str, BUFFER_SIZE);
		if (buf->size < 1)
			break ;
	}
	if (buf->size < 1 || line.size < 1)
	{
		free(buf->str);
		buf->fd = 0;
		buf->str = NULL;
		if (buf->size < 0 || line.size < 1)
			free(line.str);
		(buf->size < 0 || line.size < 1) && (line.str = NULL);
	}
