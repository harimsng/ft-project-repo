/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   parser.c                                           :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: hseong <hseong@student.42seoul.kr>         +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/10/22 23:23:21 by hseong            #+#    #+#             */
/*   Updated: 2021/10/24 20:37:19 by hseong           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../includes/ft_dict.h"
#include "../includes/ft.h"

#define		BUF_LEN		64

char		*get_dict(const char *filename);
t_dict		*alloc_arr(char *dict, int *dict_size);
void		exit_free(t_dict *dict_arr, int dict_size);

int	parser(const char *filename, t_dict **arr, int *dict_size)
{
	char		*dict;
	
	dict = get_dict(filename);
	if (!dict)
		return (-1);
	*arr = alloc_arr(dict, dict_size);
	free(dict);
	if (!*arr)
		return (-2);
	if (*dict_size < 41)
	{
		exit_free(*arr, *dict_size);
		return (-1);
	}
	return (0);
}
// !dict	> Dict Error (can be Malloc Error)
// !arr		> Malloc Error

char	*get_dict(const char *filename)
{
	char		*dict;
	char		buf[BUF_LEN];
	int			fd;
	int			read_size;
	int			dict_size;

	fd = open(filename, O_RDONLY);
	if (fd == -1)
		return (0);
	dict_size = 0;
	while ((read_size = read(fd, buf, BUF_LEN)) > 0)
		dict_size += read_size;
	if (read_size == -1 || close(fd) == -1)
		return (0);
	dict = (char *)malloc(sizeof(char) * (dict_size + 1));
	if ((fd = open(filename, O_RDONLY)) == -1 || read(fd, dict, dict_size) < 0)
		return (0);
	dict[dict_size] = 0;
	return (dict);
}

// negative keys ignored(no allocation).
// ft_split and ft_strs_to_tab used.
t_dict	*alloc_arr(char *dict, int *dict_size)
{
	char		**dict_split;
	t_dict		*ret;
	*dict_size = 0;
	dict_split = ft_split(dict, "\n");
	if (!dict_split)
		return (0);
	while (dict_split[*dict_size])
		++*dict_size;
	ret = ft_strs_to_tab(*dict_size, dict_split);
	if (!ret)
		return (0);
	return (ret);
}
// !dict_split	> Malloc Error
// !ret 		> Malloc Error

void	exit_free(t_dict *dict_arr, int dict_size)
{
	int			idx;

	idx = 0;
	while (idx < dict_size)
		free(dict_arr[idx++].value);
	free(dict_arr);
}
